...
public class DashboardFormController implements Initializable {
    ...
    private void sales () throws SQLException, ClassNotFoundException {
        ResultSet set = DBConnection.getInstance().
                getConnection().
                prepareStatement
                        ("SELECT COUNT(id_Fac) FROM factures_clients")
                .executeQuery();
        if (set.next()) {
            int customerCount = set.getInt(1);
    ...
    @FXML
    void ADDM( ActionEvent event) throws IOException {
        if (event.getSource() == ADD) {
            Parent root = FXMLLoader.load(Objects.requireNonNull(getClass().getClassLoader().getResource("AddManager/sample.fxml")));
